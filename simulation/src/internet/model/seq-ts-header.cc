  ...
  : m_seq (0)
{
  ...
  else if (IntHeader::mode == IntHeader::NORMAL)
    ih.pts = Simulator::Now().GetTimeStep();
}
...
{
	NS_ASSERT_MSG(IntHeader::mode == 1 || IntHeader::mode == IntHeader::NORMAL, "SeqTsHeader cannot GetTs when IntHeader::mode != 1");
  if (IntHeader::mode == 1) 
	  return TimeStep (ih.ts);
  else if (IntHeader::mode == IntHeader::NORMAL)
    return TimeStep (ih.pts);
}
