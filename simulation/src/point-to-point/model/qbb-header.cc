...
namespace ns3 {
	...
	void qbbHeader::SetTs(uint64_t ts){
		NS_ASSERT_MSG(IntHeader::mode == 1 || IntHeader::mode == 0, "qbbHeader cannot SetTs when IntHeader::mode != 1 or model != 0");
		if (IntHeader::mode == 1) {
			ih.ts = ts;
		} else if (IntHeader::mode == 0) {
			ih.pts = ts;
		}	
	}
	...
	uint64_t qbbHeader::GetTs() const {
		NS_ASSERT_MSG(IntHeader::mode == 1 || IntHeader::mode == 0, "qbbHeader cannot GetTs when IntHeader::mode != 1 or model != 0");
		if (IntHeader::mode == 1) {
			return ih.ts;
		} else if (IntHeader::mode == 0) {
			return ih.pts;
